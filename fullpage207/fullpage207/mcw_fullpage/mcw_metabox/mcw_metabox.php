...
if (! class_exists('MCW_MetaBox')) {
  class MCW_MetaBox {
    ...
    protected static function get_settings($meta_id) {
      ...
      if (isset(self::$_settings[$meta_id])) {
    ...
    // Get meta box value
    public static function get_field_value($meta_id, $field_id, $raw = false) {
    ...
    public function on_admin_footer() {
      ...
      echo '<script type="text/javascript">(function($){$(document).ready(function(){"use strict";';
    ...
    private function meta_box_field_html($field) {
      ...
      // Get post meta value
      $value = MCW_MetaBox::get_field_value(
        $this->id,
        $field['id'],
        ( isset($field['raw']) && !empty($field['raw']) ) ? $field['raw'] : false
      );
