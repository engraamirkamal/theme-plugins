...
function tshowcase_get_information( $id, $show, $display = array(), $singular = false ) {
	...
	$title         = false;
	$smallicons    = in_array( 'smallicons', $display, true );
	$displaygroups = in_array( 'groups', $display, true );
	$displaytax    = ( in_array( 'taxonomy', $display, true ) || in_array( 'groups2', $display ) ) && isset( $options['tshowcase_second_tax'] ) ? true : false;
	$displayctax   = in_array( 'ctaxonomy', $display, true ) && isset( $options['tshowcase_third_tax'] ) ? true : false;
	$displaydtax   = in_array( 'dtaxonomy', $display, true ) && isset( $options['tshowcase_fourth_tax'] ) ? true : false;
...
function tshowcase_get_social( $id, $show ) {
	...
	if ( $show ) {
		...
		if ( $folder === 'font' || $folder === 'font-gray' ) {
			...
			if ( $tsemailico !== '' ) {
				$html .= "<a href='" . $tsemailico . "' " . $nofollow . " target='_blank'><i class='fas fa-envelope " . $fontsize . "'></i></a>";
			}
...
function tshowcase_get_structured_data( $id, $wrap = true ) {
	...
	$html = wp_json_encode( $properties );
