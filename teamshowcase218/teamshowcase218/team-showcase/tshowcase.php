...
Plugin URI: https://cmoreira.net/team-showcase
...
function tshowcase_add_thumbnails_for_cpt() {
	...
	if ( isset( $_wp_theme_features['post-thumbnails'] ) && 1 === $_wp_theme_features['post-thumbnails'] ) {
		return;
	}

	if ( isset( $_wp_theme_features['post-thumbnails'] ) && is_array( $_wp_theme_features['post-thumbnails'][0] ) && count( $_wp_theme_features['post-thumbnails'][0] ) >= 1 ) {
		array_push( $_wp_theme_features['post-thumbnails'][0], 'tshowcase' );
		return;
	}
	if ( ! isset( $_wp_theme_features['post-thumbnails'] ) || empty( $_wp_theme_features['post-thumbnails'] ) ) {
...
if ( 'true' === $tshowcase_options['tshowcase_thumb_crop'] ) {
	$tshowcase_crop = true;
}
...
function tshowcase_filter_post_type_by_taxonomy() {
	...
	if ( $typenow === $post_type ) {
		...
		wp_dropdown_categories(
			array(
				'show_option_all' => __( 'Show All', 'tshowcase' ) . ' ' . $info_taxonomy->label,
				'taxonomy'        => $taxonomy,
				'name'            => $taxonomy,
				'orderby'         => 'name',
				'selected'        => $selected,
...
function tshowcase_columns_head( $defaults ) {
	global $post;
	if ( isset( $post->post_type ) && 'tshowcase' === $post->post_type ) {
...
function tshowcase_columns_content( $column_name, $post_ID ) {
	...
	if ( 'tshowcase' === $post->post_type ) {
		...
		if ( 'tshowcase-categories' === $column_name ) {
		...
		if ( 'tshowcase-taxonomy' === $column_name && isset( $options['tshowcase_second_tax'] ) ) {
		...
		if ( 'tshowcase-ctaxonomy' === $column_name && isset( $options['tshowcase_third_tax'] ) ) {
		...
		if ( 'tshowcase-dtaxonomy' === $column_name && isset( $options['tshowcase_fourth_tax'] ) ) {
...
function tshowcase_admin_order( $wp_query ) {
	...
	if ( $wp_query->is_post_type_archive( 'tshowcase' ) && is_admin() && ! wp_doing_ajax() ) {
...
function tshowcase_posts_per_page_admin( $wp_query ) {
	if ( is_admin() && ! wp_doing_ajax() && is_post_type_archive( 'tshowcase' ) ) {
...
function tshowcase_no_nopaging_admin( $query ) {
	if ( $query->is_post_type_archive( 'tshowcase' ) && is_admin() ) {
  ...
  <table cellpadding='2'>
	...
	foreach ( $fields as $key => $value ) {
		if ( isset( $value['label'] ) ) {
			...
			$metavalue = is_string( $metavalue ) ? $metavalue : '';
...
<table width="100%" cellpadding="0" class="tshowcase-box-social">
		...
		<tr>
		  ...
		  <td><input id="_ts<?php echo $sn[0]; ?>" size="37" name="_ts<?php echo $sn[0]; ?>" type="url" value="
									   <?php
										if ( get_post_meta( $post->ID, '_ts' . $sn[0], true ) ) {
											echo get_post_meta( $post->ID, '_ts' . $sn[0], true ); }
										?>
			" />
		...
		<tr>
		  ...
		  <td>
			<input id="_tsemailico" size="37" name="_tsemailico" type="text" value="
			<?php
			if ( $tsemailico ) {
				echo $tsemailico; }
			?>
...
function shortcode_tshowcase( $atts ) {
	...
	$jsonatts = esc_attr( json_encode( $atts ) );
	...
	if ( ! is_array( $atts ) || isset( $atts['visual_composer_team_build'] ) ) {
		...
		$category     = ( array_key_exists( 'categories', $atts ) ? $atts['categories'] : $category ); // in case atts where build with exact tax slug
		...
		$html = '<div id="tshowcase_id_' . $tshowcase_id_count . '" data-atts="' . $jsonatts . '">' . $html . '</div>';
...
function build_tshowcase( $orderby = 'menu_order', $limit = -1, $idsfilter = '0', $exclude = '0', $category = '0', $url = 'inactive', $layout = 'grid', $style = 'float-normal', $display = 'photo,name,position,email', $pagination = 'false', $imgwo = '', $searchact = 'true', $show_id = 'true', $relation = 'OR', $page = 1, $id = '0', $taxonomy = '0', $ctaxonomy = '0', $dtaxonomy = '0', $metakey = '', $offset = '', $sorder = '', $atts = array(), $meta_query = '', $filtersource = 'categories', $searchterm = '', $site = false ) {
	...
	if ( ( $orderby === 'title' || $orderby === 'name' || $orderby === 'menu_order' || $orderby === 'meta_value' || $orderby === 'meta_value_num' || $orderby === 'last_word_name' ) && $sorder === '' ) {
	...
	if ( isset( $_GET['tshowcase-categories'] ) && $_GET['tshowcase-categories'] !== '' && $searchact === 'true' ) {
		...
		foreach ( $catarray as $catdisplay ) {
			$cat_obj   = get_term_by( 'slug', $catdisplay, 'tshowcase-categories' );
			$catlabel .= '  <i>' . $cat_obj->name . '</i>';
		}
	}
	...
	if ( isset( $_GET['tshowcase-taxonomy'] ) && $_GET['tshowcase-taxonomy'] !== '' && $searchact === 'true' ) {
		...
		foreach ( $taxarray as $taxdisplay ) {
			$tax_obj   = get_term_by( 'slug', $taxdisplay, 'tshowcase-taxonomy' );
			$taxlabel .= '  <i>' . $tax_obj->name . '</i>';
		}
	}
	...
	if ( isset( $_GET['tshowcase-ctaxonomy'] ) && $_GET['tshowcase-ctaxonomy'] !== '' && $searchact === 'true' ) {
		...
		foreach ( $taxarray as $taxdisplay ) {
			$tax_obj    = get_term_by( 'slug', $taxdisplay, 'tshowcase-ctaxonomy' );
			$ctaxlabel .= '  <i>' . $tax_obj->name . '</i>';
		}
	}
	if ( isset( $_GET['tshowcase-dtaxonomy'] ) && $_GET['tshowcase-dtaxonomy'] !== '' && $searchact === 'true' ) {
		...
		foreach ( $taxarray as $taxdisplay ) {
			$tax_obj    = get_term_by( 'slug', $taxdisplay, 'tshowcase-dtaxonomy' );
			$dtaxlabel .= '  <i>' . $tax_obj->name . '</i>';
		}
	}
	...
	if ( isset( $_GET['search'] ) ) {
		...
		if ( isset( $_GET['tshowcase-categories'] ) && $_GET['tshowcase-categories'] !== '' || $_GET['search'] !== '' ) {
	...
	if ( $search !== '' && $searchmeta ) {
		...
		if ( isset( $taxonomy ) && $taxonomy !== '0' && $taxonomy !== '' ) {
			...
			$field = 'slug';
			if ( filter_var( $taxonomy, FILTER_VALIDATE_INT ) ) {
				$field = 'term_id';
			}
		...
		if ( isset( $ctaxonomy ) && $ctaxonomy !== '0' && $ctaxonomy !== '' ) {
			...
			$field = 'slug';
			if ( filter_var( $ctaxonomy, FILTER_VALIDATE_INT ) ) {
				$field = 'term_id';
			}
		...
		if ( isset( $dtaxonomy ) && $dtaxonomy !== '0' && $dtaxonomy !== '' ) {
			...
			$field = 'slug';
			if ( filter_var( $dtaxonomy, FILTER_VALIDATE_INT ) ) {
				$field = 'term_id';
			}
	...
	if ( $layout === 'pager' || $layout === 'thumbnails' ) {
		...
		if ( in_array( 'filter', $display ) || in_array( 'enhance-filter', $display ) || in_array( 'isotope-filter', $display ) || in_array( 'dropdown-filter', $display ) ) {
			...
			if ( in_array( 'isotope-filter', $display ) ) {
				$pagerfilteractive .= ' tshowcase-isotope'; } else {
		...
		$wrapclass = '';
		if ( $theme !== 'default' ) {
			$wrapclass .= ' tshowcase-pager-' . $theme . '-wrap';  }
	...
	if ( $layout === 'grid' ) {
		...
		if ( in_array( 'filter', $display ) || in_array( 'enhance-filter', $display ) || in_array( 'isotope-filter', $display ) || in_array( 'dropdown-filter', $display ) ) {
			...
			// $html .= tshowcase_build_categories_filter($display,$category);
			if ( in_array( 'isotope-filter', $display ) ) {
				$boxstyle .= ' tshowcase-isotope';} else {
	...
	if ( $layout === 'hover' ) {
		...
		if ( $theme !== 'default' ) {
			$wrapstyle .= ' tshowcase-' . $theme . '-wrap'; }
		...
		while ( $loop->have_posts() ) :
			...
			// add _tsposition below image
			if ( in_array( 'hoverposition', $display ) ) {
				$html .= "<div class='tshowcase-single-position'>" . get_post_meta( $id, '_tsposition', true ) . '</div>';
			}
...
function tshowcase_build_table_layout( $loop, $url, $display, $style, $category ) {
	...
	if ( in_array( 'filter', $display ) || in_array( 'enhance-filter', $display ) || in_array( 'isotope-filter', $display ) || in_array( 'dropdown-filter', $display ) ) {
		...
		// $html .= tshowcase_build_categories_filter($display,$category);
		if ( in_array( 'isotope-filter', $display ) ) {
			$txtstyle .= ' tshowcase-isotope';} else {
			$txtstyle .= ' tshowcase-filter-active'; }
	}

	$isot = '';
	if ( in_array( 'isotope-filter', $display ) ) {
		$isot = ' tshowcase-isotope-wrap';}
	...
	while ( $loop->have_posts() ) :
		...
		$terms = get_the_terms( $id, 'tshowcase-taxonomy' );
		if ( is_array( $terms ) ) {
			foreach ( $terms as $term ) {
				$cat .= 'ts-' . $term->slug . ' ' . 'ts-id-' . $term->term_id . ' ';
			}
		...
		$terms = get_the_terms( $id, 'tshowcase-ctaxonomy' );
		if ( is_array( $terms ) ) {
			foreach ( $terms as $term ) {
				$cat .= 'ts-' . $term->slug . ' ' . 'ts-id-' . $term->term_id . ' ';
			}
		...
		$terms = get_the_terms( $id, 'tshowcase-dtaxonomy' );
		if ( is_array( $terms ) ) {
			foreach ( $terms as $term ) {
				$cat .= 'ts-' . $term->slug . ' ' . 'ts-id-' . $term->term_id . ' ';
			}
		...
		if ( in_array( 'name', $display ) ) {
			...
			} elseif ( $url === 'inactive' || $url === 'full_image' ) {
				$info_array['title'] = '<td>' . $title . '</td>';
			} else {
				$info_array['title'] = '<td>' . $title . '</td>';
			}
		...
		if ( in_array( 'groups', $display ) ) {
			...
			if ( $tsgroups !== '' ) {
				$info_array['groups'] .= $icongroups . $tsgroups; }
		...
		if ( in_array( 'taxonomy', $display ) && isset( $options['tshowcase_second_tax'] ) ) {
			...
			if ( $tsgroups !== '' ) {
				$info_array['taxonomy'] .= $icontax . $tsgroups; }
		...
		if ( in_array( 'ctaxonomy', $display ) && isset( $options['tshowcase_third_tax'] ) ) {
			...
			if ( $tsgroups !== '' ) {
				$info_array['ctaxonomy'] .= $iconctax . $tsgroups; }
		...
		if ( in_array( 'dtaxonomy', $display ) && isset( $options['tshowcase_fourth_tax'] ) ) {
			...
			if ( $tsgroups !== '' ) {
				$info_array['dtaxonomy'] .= $icondtax . $tsgroups; }
		...
		foreach ( $fields as $key => $value ) {
			...
			if ( in_array( $key, $display ) ) {
				...
				if ( ! isset( $value['hide'] ) ) {
					...
					} elseif ( isset( $value['format'] ) && $value['format'] === 'tel' ) {
					...
					} elseif ( isset( $value['format'] ) && $value['format'] === 'url' ) {
...
function tshowcase_add_theme( $theme, $layout ) {
			...
			wp_register_style( $thadd['name'], plugins_url( $thadd['link'], __FILE__ ), array(), $version, 'all' );
...
function tshowcase_default_layout() {
	...
	$version = $plugin['version'];
			...
			wp_register_style( 'tshowcase-normal-style', plugins_url( 'css/normal.css', __FILE__ ), array(), $version, 'all' );
...
function tshowcase_add_lightbox_files() {
	...
	wp_register_script( 'tshowcase-featherlight', plugins_url( 'js/featherlight/featherlight.min.js', __FILE__ ), array( 'jquery' ), $version, true );
	wp_enqueue_script( 'tshowcase-featherlight' );

	wp_deregister_style( 'tshowcase-featherlight' );
	wp_register_style( 'tshowcase-featherlight', plugins_url( 'js/featherlight/featherlight.min.css', __FILE__ ), array(), $version, 'all' );
...
function tshowcase_pager_layout( $lshowcase_slider_count ) {
	...
	wp_register_script( 'tshowcase-bxslider', plugins_url( 'js/bxslider/jquery.bxslider.js', __FILE__ ), array( 'jquery' ), $version, false );
	wp_enqueue_script( 'tshowcase-bxslider' );

	wp_deregister_script( 'tshowcase-bxslider-pager' );
	wp_register_script( 'tshowcase-bxslider-pager', plugins_url( 'js/pager.js', __FILE__ ), array( 'jquery', 'tshowcase-bxslider' ), $version, false );
...
function shortcode_tshowcase_ajax( $array ) {
	...
	if ( isset( $_POST['loadmore'] ) && $_POST['loadmore'] === '1' ) {
...
function tshowcase_ajax_pagination( $tshowcase_atts ) {
	...
	$version = $plugin['version'];
	  ...
	  wp_register_script( 'tshowcase-ajax-pager', plugins_url( 'js/ajax-pagination.js', __FILE__ ), array( 'jquery' ), $version, false );
...
function tshowcase_ajax_loadmore( $tshowcase_atts ) {
	...
	$version = $plugin['version'];
	  ...
	  wp_register_script( 'tshowcase-ajax-loadmore', plugins_url( 'js/ajax-loadmore.js', __FILE__ ), array( 'jquery' ), $version, false );
...
function tshowcase_filter_code() {
	...
	wp_register_script( 'tshowcase-filter', plugins_url( '/js/filter.js', __FILE__ ), array( 'jquery', 'jquery-ui-core', 'jquery-effects-core' ), $version, false );
...
function tshowcase_dropdown_code() {
	...
	wp_register_script( 'tshowcase-dropdown-filter', plugins_url( '/js/dropdown-filter.js', __FILE__ ), array( 'jquery', 'jquery-ui-core', 'jquery-effects-core' ), $version, false );
...
function tshowcase_enhance_filter_code() {
	...
	wp_register_script( 'tshowcase-enhance-filter', plugins_url( '/js/filter-enhance.js', __FILE__ ), array( 'jquery', 'jquery-effects-core' ), $version, false );
...
function tshowcase_isotope_filter_code() {
	...
	wp_register_script( 'tshowcase-isotope', plugins_url( '/js/isotope.pkgd.min.js', __FILE__ ), array( 'jquery' ), $version, false );
	wp_enqueue_script( 'tshowcase-isotope' );

	wp_deregister_script( 'tshowcase-cells-isotope' );
	wp_register_script( 'tshowcase-cells-isotope', plugins_url( '/js/cells-by-row.js', __FILE__ ), array( 'jquery', 'tshowcase-isotope' ), $version, false );
	wp_enqueue_script( 'tshowcase-cells-isotope' );

	wp_deregister_script( 'tshowcase-isotope-filter' );
	wp_register_script( 'tshowcase-isotope-filter', plugins_url( '/js/filter-isotope.js', __FILE__ ), array( 'jquery', 'tshowcase-isotope' ), $version, false );
	wp_enqueue_script( 'tshowcase-isotope-filter' );

	wp_deregister_script( 'tshowcase-imgs-loaded' );
	wp_register_script( 'tshowcase-imgs-loaded', plugins_url( '/js/imagesloaded.pkgd.min.js', __FILE__ ), array( 'jquery', 'tshowcase-isotope' ), $version, false );
...
function tshowcase_add_global_css() {
	...
	wp_register_style( 'tshowcase-global-style', plugins_url( '/css/global.css', __FILE__ ), array(), $version, 'all' );
...
function tshowcase_add_smallicons_css() {
	...
	$version = $plugin['version'];
	  ...
	  wp_deregister_style( 'tshowcase-smallicons' );
		  wp_register_style( 'tshowcase-smallicons', plugins_url( '/css/font-awesome/css/fontawesome-all.min.css', __FILE__ ), array(), $version, 'all' );
	...
	if ( isset( $options['tshowcase_single_show_posts'] ) ) {
		...
		if ( $tsuser !== '0' ) {
			...
			if ( $tshowcase_posts_query->have_posts() ) {
				...
				$html .= '<h3>' . $title . '</h3>';
				$html .= '<ul>';
				while ( $tshowcase_posts_query->have_posts() ) :
					$tshowcase_posts_query->the_post();
...
function tshowcase_defaults() {
	...
	if ( ! is_array( $tmp ) ) {
		...
		$arr = array(
			'tshowcase_name_singular'        => 'Member',
...
function tshowcase_build_categories_filter( $display, $source, $category, $taxonomy, $ctaxonomy, $dtaxonomy ) {
	...
	foreach ( $use_tax as $key => $tax ) {
		...
		$all = isset( $opts['tshowcase_all_label'] ) ? $opts['tshowcase_all_label'] : $ts_labels['filter']['all-entries-label'];

		if ( function_exists( 'icl_object_id' ) ) {
			$all = apply_filters( 'wpml_translate_single_string', $all, 'tshowcase', 'Settings Field: ' . $all );
		}
...
function set_tshowcase_order( $wp_query ) {
	...
	if ( is_admin() && ! wp_doing_ajax() && $wp_query->is_post_type_archive( 'tshowcase' ) && isset( $post_type ) && $post_type === 'tshowcase' ) {
...
function tshowcase_team_count() {
	$count = wp_count_posts( 'tshowcase' );
...
/**
 * Ensure that the plugin is never updated from WordPress repository. This works by removing the
 * plugin from the list of available updates.
 */
/*
add_filter( 'http_request_args', 'tshowcase_prevent_updates' , 10, 2 );
function tshowcase_prevent_updates ( $response, $url ) {
