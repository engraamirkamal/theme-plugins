...
if ( ! class_exists( 'tshowcase_ordering' ) ) {
	class tshowcase_ordering {
		...
		public static function wp() {
			...
			if ( 0 === strpos( get_query_var( 'orderby' ), 'menu_order' ) ) {
				...
				if ( ! isset( $_GET['filter_action'] ) && ! isset( $_GET['s'] ) ) {

					$script_name = 'js/order/simple-page-ordering.js';
					wp_enqueue_script( 'tshowcase-simple-page-ordering', plugins_url( $script_name, __FILE__ ), array( 'jquery-ui-sortable' ), '2.1', true );
					wp_enqueue_style( 'tshowcase-simple-page-ordering', plugins_url( 'js/order/simple-page-ordering.css', __FILE__ ) );
		...
		public static function admin_head() {
			...
			$screen->add_help_tab(
				array(
					...
					'title'   => __( 'Simple Page Ordering', 'tshowcase' ),
					'content' => '<p>' . __( 'To reposition an item, simply drag and drop the row by "clicking and holding" it anywhere (outside of the links and form controls) and moving it to its new position.', 'tshowcase' ) . '</p>',
				)
			);
		}
		...
		public static function sort_by_order_link( $views ) {
			...
			if ( ! isset( $_GET['filter_action'] ) && ! isset( $_GET['s'] ) ) {
				...
				if ( get_query_var( 'orderby' ) != 'menu_order' ) {
					...
					$views['byorder'] = sprintf( '<a href="%s" class="%s">%s</a>', $query_string, $class, __( 'Sort by Order', 'tshowcase' ) );

				} else {

					$views['byorder'] = __( 'Drag & Drop Ordering Activated', 'tshowcase' );
	...
	tshowcase_ordering::get_instance();
